...
const RightSection = ({idx, link, image, title, description,source, items = [] }) => {
        ...
        <div className={clsx("container", styles.container_section)}>
          ...
          <div className={clsx("row", styles.right_section)}>
              {data?.items.map((props, idx) => (
                <RightSection key={idx} idx={idx} {...props} source={location?.pathname} items={props.items} />
            ))}
          </div>
        </div>
    </section>
...
const ToggleTextButton = ({ expanded, description, onClick, title }) => {
  ...
  const label =
    expanded && description.length > 105 ? `Show less ${title}` : `Show more ${title}`;
    return (
      <p
        onClick={onClick}
        className={clsx("text-sm text-emerald-600", styles.toggleButton)}
      >
        {label}
      </p>
    );
  ...
  return (
    <>
      {parts.map((part, index) => {
        ...
        return <span key={index}>{part.length > 105 && !expanded
                    ? `${part.substring(0, 105)}...`
                    : part}</span>;
      })}
    </>
  );
